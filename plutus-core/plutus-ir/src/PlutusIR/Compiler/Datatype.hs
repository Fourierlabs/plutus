...
mkDatatypeSumTy :: ann -> Datatype TyName Name uni ann -> Type TyName uni ann
mkDatatypeSumTy ann (Datatype _ _ _ _ constrs) =
    let constrArgs = fmap (mkConstrProdTy ann) constrs
    in TySum ann constrArgs

mkConstrProdTy :: a -> VarDecl tyname name uni a -> Type tyname uni a
mkConstrProdTy ann vd =
...
-- FIXME: inline this
mkDatatypePatternFunctor :: MonadQuote m => DatatypeStyle -> ann -> Datatype TyName Name uni ann -> m (Type TyName uni ann)
mkDatatypePatternFunctor ScottEncoding ann d  = mkScottTy ann d <$> resultTypeName d
mkDatatypePatternFunctor SumsOfProducts ann d = pure $ mkDatatypeSumTy ann d

mkBoundDatatypePatternFunctor :: MonadQuote m => DatatypeStyle -> ann -> Datatype TyName Name uni ann -> m (Type TyName uni ann)
mkBoundDatatypePatternFunctor style ann d@(Datatype _ tv _ _ _) = do
    st <- mkDatatypePatternFunctor style ann d
...
-- @
mkConstructor :: MonadQuote m => DatatypeStyle -> PLCRecType uni fun a -> Datatype TyName Name uni (Provenance a) -> Int -> m (PIRTerm uni fun a)
mkConstructor style dty d@(Datatype ann _ tvs _ constrs) index = do
    ...
    constrBody <- case style of
          SumsOfProducts -> do
            boundPf <- mkBoundDatatypePatternFunctor style ann d
...
-- @
mkDestructor :: MonadQuote m => DatatypeStyle -> PLCRecType uni fun a -> Datatype TyName Name uni (Provenance a) -> m (PIRTerm uni fun a)
mkDestructor style dty d@(Datatype ann _ tvs _ constrs) = do
...
compileDatatype r body d = do
    style <- view (ccOpts . coDatatypes . dcoStyle)
    p <- getEnclosing

    (concreteTyDef, constrDefs, destrDef) <- compileDatatypeDefs style r d
...
compileDatatypeDefs
    ...
    -> m (PLC.Def (TyVarDecl TyName (Provenance a)) (PLCRecType uni fun a),
          ...
          PLC.Def (VarDecl TyName Name uni (Provenance a)) (PIRTerm uni fun a))
compileDatatypeDefs style r d@(Datatype ann tn _ destr constrs) = do
    -- we compute the pattern functor and pass it around to avoid recomputing it
    pf <- mkDatatypePatternFunctor style ann d
    concreteTyDef <- PIR.Def tn <$> mkDatatypeType r pf d

    constrDefs <- for (zip constrs [0..]) $ \(c, i) -> do
        let constrTy = mkConstructorType d c
        PIR.Def (VarDecl (DatatypeComponent Constructor ann) (_varDeclName c) constrTy) <$> mkConstructor style (PIR.defVal concreteTyDef) d i

    destrDef <- do
        resultType <- resultTypeName d
        let destTy = fmap (DatatypeComponent DestructorType) (mkDestructorTy (mkScottTy ann d resultType) d)
        t <- mkDestructor style (PIR.defVal concreteTyDef) d
